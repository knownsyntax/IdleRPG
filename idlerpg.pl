    ...
    # don't penalize /notices to the bot
        ...
        if (lc($arg[2]) eq lc($opts{botnick})) { # to us, not channel
            ...
            if ($arg[3] eq "\1version\1") {
                notice("\1VERSION IdleRPG v$version by jotun; ".
            ...
            elsif ($arg[3] eq "reset") {
                if (!ha($username)) {
                    privmsg("You don't have access to RESET.", $usernick);
                }
                elsif (!$pausemode) {
                    privmsg("ERROR: Can only use RESET while in PAUSE mode.",
                            $usernick, 1);
                }
                else {
                    for my $k (keys(%rps)) {
            ...
            #
                ...
                if (!ha($username)) {
                   privmsg("You do not have access.", $usernick, 1);
                } else {
                   privmsg("Okay.", $usernick, 1);
